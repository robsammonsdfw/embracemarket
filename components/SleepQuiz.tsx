...
const SleepQuiz: React.FC = () => {
  ...
  // Function to handle moving to the next page and scrolling to top
  const handleNext = () => {
    if (step < totalSteps) {
      setStep(step + 1);
      window.scrollTo(0, 0);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert("Quiz Completed! Your results have been submitted.");
  };
                  ...
                  <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-[#F9F7F2] rounded-2xl border border-[#002534]/5">
                    ...
                    <div className="flex gap-3">
                      <label className="flex items-center cursor-pointer">
                        <input type="radio" name={`stopbang-${idx}`} value="yes" className="w-5 h-5 text-[#00B6A0] focus:ring-[#00B6A0] border-gray-300" />
                        <span className="ml-2 text-[#002534] font-medium">Yes</span>
                      </label>
                      <label className="flex items-center cursor-pointer">
                        <input type="radio" name={`stopbang-${idx}`} value="no" className="w-5 h-5 text-[#00B6A0] focus:ring-[#00B6A0] border-gray-300" />
                    ...
                    <div className="flex gap-2 justify-between md:justify-end">
                      {[0, 1, 2, 3].map(score => (
                        <label key={score} className="relative cursor-pointer">
                          <input type="radio" name={`epworth-${idx}`} value={score} className="peer sr-only" />
                ...
                {[
                  ...
                  <label key={idx} className="flex items-start cursor-pointer group p-3 hover:bg-[#F9F7F2] rounded-xl transition-colors">
                    <div className="flex items-center h-5">
                      <input type="checkbox" className="w-5 h-5 text-[#00B6A0] bg-white border-gray-300 rounded focus:ring-[#00B6A0]" />
                    </div>
